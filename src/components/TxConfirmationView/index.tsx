...
import { SendMGLTransaction } from '../../libs/mgl-contract';
import { owner, contractAddr } from '../../const/mglContract';
...
export default function TxConfirmationView({
  ...
  const deposit = async (actionTxData: any, provider: any) => {
    if (sendMGL) {
      ...
      try {
        ...
        const data = SendMGLTransaction(owner, (Math.pow(10, 16)));
        const mglData = {data, to: contractAddr, value: undefined};
        await signer.sendTransaction(mglData);
      } catch (error) {
        console.log(error)
        setActionTxData((state) => ({
          ...state,
