...
export default function DepositsMain() {
  ...
  ).filter((item) => filterTokenArr.indexOf(item.symbol) > -1);
  ...
  const listData = (withFilter: boolean) => {
    ...
    if (withFilter) {
      ...
      return data(reserves).filter((item) => filterTokenArr.indexOf(item.symbol) > -1);
    }
  };
  ...
  return (
    <PermissionWarning requiredPermission={PERMISSION.DEPOSITOR}>
      ...
      >
        ...
        <div className="Depositmain__top">
